...
##--- Global Variables ---
##------------------------------------------------------
CITY="latitude=-51.5&longitude=-0.120000124"
...
if [ $(echo -n $JSONRAW | jq empty >  /dev/null 2>&1; echo $?) -eq 0 ]; then
	...
	echo $JSONRAW | jq '.daily.time[]' | tr -d '"' | awk '{print "LON_"$1}' > $WORKFOLDER/id.csv
	echo $JSONRAW | jq '.daily.time[]' | tr -d '"' | awk '{print "London"}' > $WORKFOLDER/city.csv
